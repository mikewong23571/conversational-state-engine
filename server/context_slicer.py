...
class ContextSlicer:
    ...
    def slice_state(self, state: Dict[str, Any], intent: str = None) -> List[ContextSlice]:
        """根据意图对状态进行智能切片"""
        # 1. 分析意图，确定相关路径
    ...
    def _generate_slice_candidates(self, state: Dict[str, Any], 
                                 relevant_paths: List[str]) -> List[ContextSlice]:
        """生成切片候选"""
        candidates = []
    ...
    def _slice_by_path_pattern(self, state: Dict[str, Any], 
                              pattern: str) -> List[ContextSlice]:
        """根据路径模式生成切片"""
        slices = []
    ...
    def _collect_dependency_chain(self, start_key: str, dep_graph: Dict[str, Any], 
    ...
    def _rank_slices(self, slices: List[ContextSlice], intent: str = None) -> List[ContextSlice]:
        """对切片进行评分和排序"""
        # 计算综合得分
    ...
    def _calculate_intent_relevance(self, slice_obj: ContextSlice, intent: str) -> float:
        """计算切片与意图的相关性"""
        if not intent:
            return 0
        ...
        # 检查路径相关性
        if "/stories" in slice_obj.path and any(word in intent_lower for word in ["story", "需求", "功能"]):
            relevance += 0.8
        if "/glossary" in slice_obj.path and any(word in intent_lower for word in ["术语", "glossary"]):
            relevance += 0.6
        if "priority" in slice_obj.path and any(word in intent_lower for word in ["priority", "优先级"]):
            relevance += 0.7
    ...
    def _extract_slice_data(self, data: Any, max_depth: int = 3) -> Any:
        ...
        if isinstance(data, dict):
            ...
            return [self._extract_slice_data(item, max_depth - 1) for item in data[:5]]  # 限制列表长度
        else:
            return data
...
def example_usage():
    ...
    for slice_obj in slices:
        print(f"- {slice_obj.id}: 重要性={slice_obj.importance_score:.2f}, 大小={slice_obj.size}")
