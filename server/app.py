...
from .analyzer import MockAnalyzer, LLMAnalyzer
from .context_slicer import ContextSlicer, SliceConfig
from .auth import (
    init_auth_db, authenticate_user, create_user, get_current_user,
    require_permission, check_session_access, grant_session_access,
    create_access_token, User, UserLogin, UserCreate, Token
)
...
if llm_provider.lower() == "openai" and (os.getenv("OPENAI_API_KEY") or os.getenv("CSE_API_KEY")):
    # 支持OpenAI兼容的API提供商
    analyzer_kwargs = {}
    if os.getenv("CSE_MODEL"):
        analyzer_kwargs["model"] = os.getenv("CSE_MODEL")
    if os.getenv("CSE_BASE_URL") or os.getenv("OPENAI_BASE_URL"):
        analyzer_kwargs["base_url"] = os.getenv("CSE_BASE_URL") or os.getenv("OPENAI_BASE_URL")
    ...
    print("Using mock analyzer (set CSE_LLM_PROVIDER=openai and OPENAI_API_KEY/CSE_API_KEY to use OpenAI-compatible API)")
...
async def analyze_message(
    ...
    with get_db() as conn:
        ...
        context_slices = context_slicer.slice_state(
            {"data": current_state}, 
            message
        )
    ...
    result = {
        ...
        "analyzer_type": "openai" if hasattr(analyzer, 'client') and analyzer.client else "mock",
        "context_analysis": {
            ...
            if risk_level != "high":
                ...
                conn.execute(
                    ...
                    (commit_id, sid, session["current_version"], new_version,
                     json.dumps(patches), json.dumps(reverse_patches), f"Auto-applied: {message}")
                )
...
async def propose_patches(
            ...
            try:
                ...
                if auto_fix_patches:
                    ...
                    if not fix_validation.is_valid:
                        ...
                        auto_fix_patches = [p for i, p in enumerate(auto_fix_patches) 
                                          if not any(f"patch[{i}]" in err.get("path", "") for err in fix_validation.errors)]
    ...
    response.update({
        ...
        "conflict_resolution": {
            ...
            "high_priority_fixes": len([f for f in auto_fix_patches if any(keyword in f.get("reason", "").lower() for keyword in ["priority", "auth", "dependency"])]),
...
async def resolve_conflicts(
    ...
    with get_db() as conn:
        ...
        if apply_fixes and prioritized_fixes:
            ...
            # 重新检测冲突以验证修复效果
            remaining_conflicts = conflict_detector.detect_with_patches(full_state, updated_patches)
            ...
            return {
                ...
                "fix_summary": {
                    "high_priority": len([f for f in prioritized_fixes if any(kw in f.get("reason", "").lower() for kw in ["priority", "auth", "dependency"])]),
                    "structural": len([f for f in prioritized_fixes if "structural" in f.get("reason", "").lower()]),
                    "timeline": len([f for f in prioritized_fixes if "timeline" in f.get("reason", "").lower()])
                }
            }
...
async def validate_session_state(
    ...
    with get_db() as conn:
        ...
        return {
            ...
            "overall_health": {
                "status": "healthy" if validation_result.is_valid else "issues_detected",
                "error_count": len(validation_result.errors),
                "warning_count": len(validation_result.warnings),
                "suggestion_count": len(validation_result.suggestions)
            },
            "recommendations": {
                "critical_fixes_needed": len([e for e in validation_result.errors if e.get("severity") == "error"]),
...
async def validate_patches_endpoint(
    ...
    return {
        ...
        "safety_assessment": {
            ...
            "destructive_operations": len([p for p in patches if p.get("op") == "remove"]),
            "requires_review": not validation_result.is_valid or any(p.get("op") in ["remove", "move"] for p in patches)
        }
    }
...
async def confirm_changes_stage(
    ...
    with get_db() as conn:
        ...
        if selected_patch_indices is not None:
            # 过滤选中的patches
            selected_patches = [patches[i] for i in selected_patch_indices if i < len(patches)]
        else:
            # 默认选择所有patches
        ...
        conn.execute(
            """UPDATE patch_proposals 
               ...
               WHERE id = ?""",
            (json.dumps(stage_confirmations), json.dumps(selected_patch_indices), proposal_id)
        )
        conn.commit()
...
async def confirm_side_effects_stage(
    ...
    with get_db() as conn:
        ...
        if apply_auto_fixes:
            ...
            patches = json.loads(proposal_row["patches"])
            selected_patches = [patches[i] for i in selected_indices if i < len(patches)]
...
async def get_proposal_status(
    ...
    with get_db() as conn:
        ...
        return {
            ...
            "selected_patch_count": len(selected_indices) if selected_indices else len(json.loads(proposal_row["patches"])),
