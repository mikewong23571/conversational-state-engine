...
# Allow numeric characters in story key prefixes (e.g., "E2E-Login")
StoryKey = Annotated[str, Field(pattern=r"^[A-Z0-9]+-[A-Za-z0-9]+$")]
...
class IntentionSet(BaseModel):
    ...
    def validate_intention_conflicts(cls, v):
        ...
        for intention in v:
            ...
            if path in paths:
                ...
                if (
                    existing_action == Action.delete
                    and intention.action != Action.delete
                ):
                    raise ValueError(
                        f"Cannot {intention.action} on path {path} after delete"
                    )
                elif (
                    intention.action == Action.delete
                    and existing_action != Action.delete
                ):
                    raise ValueError(
                        f"Cannot delete path {path} after {existing_action}"
                    )
    ...
    def validate_set_consistency(cls, values):
        ...
        if high_confidence_count == 0 and len(items) > 0:
            ...
            values["notes"] = (
                current_notes + " Note: Low confidence intentions."
            ).strip()
...
class Story(BaseModel):
    ...
    def validate_story_consistency(cls, values):
        ...
        if auth_type == AuthType.sso and "password" in criteria_text:
            raise ValueError(
                "SSO auth_type conflicts with password-related acceptance criteria"
            )
        elif auth_type == AuthType.password and "sso" in criteria_text:
            raise ValueError(
                "Password auth_type conflicts with SSO-related acceptance criteria"
            )
    ...
    def validate_story_dependencies(cls, v):
        ...
        for story in v:
            for dep in story.dependencies:
                if dep not in story_keys:
                    raise ValueError(
                        f"Story {story.key} depends on non-existent story {dep}"
                    )
        ...
        for story in v:
            if has_circular_dependency(story.key):
                raise ValueError(
                    f"Circular dependency detected involving story {story.key}"
                )
