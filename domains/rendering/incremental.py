...
class IncrementalRenderer:
    ...
    def _render_fragment(
        self, state: dict, fragment_id: str, template_name: str
    ) -> str:
        """渲染单个片段"""
...
"""
    ...
    def _render_story_detail(self, state: dict) -> str:
        ...
        for story in stories:
            content += (
                f"### {story.get('key', 'NO-KEY')}: {story.get('title', '无标题')}\n\n"
            )
    ...
    def _assemble(self, template_name: str, state: dict) -> str:
        ...
        # 定义片段顺序
        fragment_order = ["header", "summary", "story_list", "story_detail", "glossary"]
